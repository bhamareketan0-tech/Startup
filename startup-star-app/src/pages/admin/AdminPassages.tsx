          ...
          {passages.map((p) => (
            <div key={p.id} className="bg-[#0d1b2a] border border-white/10 rounded-2xl p-5 hover:border-white/20 transition-colors">
              <div className="flex items-start gap-3 mb-3">
                <FileText className="w-4 h-4 text-[#00d4ff] mt-0.5 shrink-0" />
                <div className="flex-1 min-w-0">
                  <h3 className="text-white font-semibold text-sm">{p.title}</h3>
                  <div className="flex gap-2 mt-1">
                    <span className="px-1.5 py-0.5 bg-[#00d4ff]/10 text-[#00d4ff] rounded text-xs border border-[#00d4ff]/20">Class {p.class}</span>
                    {p.chapter && <span className="px-1.5 py-0.5 bg-white/5 text-white/40 rounded text-xs border border-white/10">{p.chapter}</span>}
                    <span className={`px-1.5 py-0.5 rounded text-xs border ${p.status === "active" ? "bg-[#00ffb3]/10 text-[#00ffb3] border-[#00ffb3]/20" : "bg-white/5 text-white/30 border-white/10"}`}>{p.status || "active"}</span>
                  </div>
                </div>
              </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input value={editingP.title || ""} onChange={(e) => setEditingP({ ...editingP, title: e.target.value })}
                  className="w-full bg-[#111e30] border border-white/10 rounded-xl px-3 py-2 text-white text-sm focus:outline-none focus:border-[#00ffb3]/50" />
              </div>
              ...
              <div>
                ...
                <textarea value={editingP.body || ""} onChange={(e) => setEditingP({ ...editingP, body: e.target.value })}
                  ...
                  className="w-full bg-[#111e30] border border-white/10 rounded-xl px-3 py-2 text-white text-sm resize-none focus:outline-none focus:border-[#00ffb3]/50 placeholder-white/20" />
              </div>
            </div>
            <div className="flex gap-3 mt-5 pt-4 border-t border-white/10">
              <button onClick={save} disabled={saving}
                className="flex items-center gap-2 px-6 py-2 bg-gradient-to-r from-[#00ffb3] to-[#00d4ff] text-black font-semibold rounded-xl text-sm hover:opacity-90 disabled:opacity-50">
