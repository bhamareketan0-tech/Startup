...
const PLAN_STYLES: Record<string, string> = {
  elite: "text-[#00FF9D] bg-[#00FF9D]/10 border-[#00FF9D]/30",
  pro: "text-[#00FF9D] bg-[#00FF9D]/10 border-[#00FF9D]/30",
...
export function AdminStudents() {
  ...
  async function openStudent(s: UserProfile) {
    ...
    try {
      const res = await api.get(`/students/${s.id}/stats`);
  ...
  async function saveStudent() {
    ...
    setSaving(true);
    try {
      const res = await api.put(`/students/${selectedStudent.id}`, { name: editName, class: editClass, plan: editPlan });
      if (res.user) {
  ...
  async function deleteStudent() {
    if (!selectedStudent) return;
    try {
      await api.del(`/students/${selectedStudent.id}`);
                ...
                <tr key={s.id} className="border-b border-white/5 hover:bg-white/3 cursor-pointer transition-colors" onClick={() => openStudent(s)}>
                  <td className="px-5 py-3">
                    <div className="flex items-center gap-2">
                      <div className="w-7 h-7 rounded-full bg-[#00FF9D]/20 flex items-center justify-center shrink-0">
                  ...
                  <td className="px-4 py-3">
                    {s.class && <span className="px-1.5 py-0.5 bg-[#00FF9D]/10 text-[#00FF9D] rounded text-xs border border-[#00FF9D]/20">Class {s.class}</span>}
                  </td>
                  <td className="px-4 py-3">
          ...
          <div className="w-88 bg-[#0a1628] border-l border-white/10 flex flex-col overflow-hidden" style={{ width: 340 }}>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4">
              ...
              <div className="text-center pb-2">
                <div className="w-16 h-16 rounded-full bg-[#00FF9D]/20 flex items-center justify-center mx-auto mb-3 text-xl font-bold text-[#00FF9D]">
                ...
                {editing ? (
                  <input
                    ...
                    className="text-center bg-white/5 border border-white/20 rounded-lg px-3 py-1.5 text-white font-semibold w-full text-sm focus:outline-none focus:border-[#00FF9D]/50"
              ...
              {/* Success toast */}
              {saved && (
                <div className="flex items-center gap-2 bg-[#00FF9D]/10 border border-[#00FF9D]/20 rounded-xl px-3 py-2 text-[#00FF9D] text-xs">
                  ...
                  <div className="flex gap-2">
                    {["11", "12"].map((c) => (
                      <button key={c} onClick={() => setEditClass(c)}
                        ...
                        style={editClass === c
                          ? { background: "#00FF9D", color: "black", borderColor: "#00FF9D" }
              ...
              {editing ? (
                <div className="flex gap-2">
                  <button onClick={saveStudent} disabled={saving}
                    className="flex-1 flex items-center justify-center gap-2 py-2 rounded-xl bg-[#00FF9D] text-black text-xs font-bold hover:bg-[#00e5a0] disabled:opacity-50 transition-all">
