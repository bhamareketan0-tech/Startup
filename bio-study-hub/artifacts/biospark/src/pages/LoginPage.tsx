  ...
  const handleClick = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        onError(data.error || "Google Sign-In is not available. Please use email & password.");
        return;
      }
      window.location.href = `${API_URL}/auth/google`;
    } catch {
      onError("Could not reach the server. Please try email & password login.");
    } finally {
      setChecking(false);
    }
  };
  ...
  return (
    <>
      ...
      <button
        ...
        onClick={handleClick}
        disabled={checking}
        className="mt-3 w-full flex items-center justify-center gap-3 py-3 rounded font-black uppercase text-sm transition-all disabled:opacity-60"
        style={{
