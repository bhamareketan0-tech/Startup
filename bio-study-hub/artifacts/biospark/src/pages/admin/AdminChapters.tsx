...
import {
  ...
  ChevronUp, ChevronDown, Save, AlertTriangle,
  GripVertical, BookOpen, RefreshCw
} from "lucide-react";
...
export function AdminChapters() {
  ...
  async function loadChapters() {
    ...
    setNewChapterName("");
    const data = await fetchChaptersFromAPI(cls);
    setChapters(data);
  ...
  async function handleSave() {
    ...
    try {
      await saveChaptersToAPI(cls, chapters);
      setIsDirty(false);
      setSaveStatus("saved");
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-white/40 text-sm mt-1">Changes are saved to MongoDB and reflect instantly on the main website</p>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={loadChapters} disabled={loadingChapters} className={ghostBtn}>
            <RefreshCw className={`w-3.5 h-3.5 ${loadingChapters ? "animate-spin" : ""}`} />
            Reload
          </button>
          {isDirty && (
            <button onClick={handleSave} disabled={saveStatus === "saving"} className={accentBtn}>
      ...
      {isDirty && (
        <div className="flex items-center justify-between px-4 py-3 bg-[#00FF9D]/10 border border-[#00FF9D]/25 rounded-xl">
          ...
          <div className="flex gap-2">
            <button onClick={handleSave} disabled={saveStatus === "saving"} className={accentBtn}>
              <Save className="w-3.5 h-3.5" /> {saveStatus === "saving" ? "Saving..." : "Save Changes"}
            </button>
            <button
